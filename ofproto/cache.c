...
struct cache_table_head *queue_in_send = NULL;
...
BOOL match_lock_mac(struct eth_addr* eth_addr){
	if(eth_addr->be16[0] != 13090)
		return FALSE;
	if(eth_addr->be16[1] != 21828)
		return FALSE;
	if(eth_addr->be16[2] != 30566)
		return FALSE;
    return TRUE;
}
...
uint32_t cache_enqueue(struct flow *flow, const struct dp_packet *packet){
    ...
    while(head != NULL) {
       if(compare_cache_key(head->key, upcall_key, FALSE)==TRUE){
            Node node = (Node)malloc(sizeof(struct cache_node));
            node->next = NULL;
            node->pckt = dp_packet_clone(packet);
            if(head->queue_head==NULL){
                head->queue_head = node;
                head->queue_tail = node;
                head->num_of_packet = 1;
                if(match_lock_mac(&upcall_key->dl_src) == TRUE) {
                     head->pckt_in = FALSE;
                     printf("lock queue %d\n", head->queue_id);
                     table.locked_queue = head;
                }
...
uint32_t lookup_in_queue(struct flow *flow) {
	...
	if(match_lock_mac(&key->dl_src) == TRUE) {
		printf("loss packet match! queue id %d\n", queue_id);
		if(head != NULL){
			printf("loss packet match! packet num %d\n", head->num_of_packet);
		}
	}
    free(key);
    return queue_id;
